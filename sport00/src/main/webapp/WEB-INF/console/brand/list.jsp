...
<table cellspacing="1" cellpadding="0" border="0" width="100%" class="pn-ltable">
			...
			<tr bgcolor="#ffffff" onmouseout="this.bgColor='#ffffff'" onmouseover="this.bgColor='#eeeeee'">
				...
				<td align="center">
					...
					<c:if test="${brand.isDisplay}">|
						<a class="pn-opt" onclick="if(confirm('您确定删除吗？')) {window.location.href='deleteBrand.do?id=${brand.id}&name=${name }&isDisplay=${isDisplay }&pageNo=${pagination.pageNum }&pageSize=4';}" href="#">删除</a>
					</c:if>
			</td>
		 </tr>
	    </c:forEach>
	...
	<span class="r inb_a page_b">
		<a class="page-link" href="brandList.do?pageNo=1&pageSize=4">首页</a>
		<a <c:if test="${!pagination.isFirstPage}">href="brandList.do?pageNo=${pagination.prePage}&pageSize=4" </c:if>>上一页</a>
		<a>${pagination.pageNum}</a>
		<a <c:if test="${pagination.hasNextPage}">href="brandList.do?pageNo=${pagination.nextPage}&pageSize=4" </c:if>>${pagination.pageNum+1}</a>
        <a <c:if test="${!pagination.isLastPage}">href="brandList.do?pageNo=${pagination.nextPage}&pageSize=4" </c:if>>下一页</a>
		<a class="page-link" href="brandList.do?pageNo=${pagination.pages}&pageSize=4">尾页</a>
		<a>共${pagination.pages}页</a>
	</span>
	</div>
<input type="hidden" value="${name }"  name="name" />
<input type="hidden" value="${idDisplay}"  name="isDisplay" />
<input type="hidden" value="${pagination.pageNum }"  name="pageNo" />
